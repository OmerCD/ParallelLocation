...
namespace ReflectorO
{
    public static class MultidimensionalArrayExtensions
    {
        ...
        private static void ArrayForEach<T>(int[] dimensionSizes, Action<T, int[]> action, int[] externalCoordinates,
            Array masterArray)
        {
            if (dimensionSizes.Length == 1)
                ...
                for (int i = 0; i < dimensionSizes[0]; i++)
                    ArrayForEach(dimensionSizes.Skip(1).ToArray(), action,
                        externalCoordinates.Concat(new[] {i}).ToArray(), masterArray);
        }
    }
}